...
class Transformations extends Component {
  render(){
    ...
    return (
      <Grid item container style={{...Styles.highZ, ...Styles.greyBG}} alignItems="flex-start" justify="center" className="top">
        <Grid item>
        ...
        <Grid item container xs={10} justify="space-evenly" spacing={2}>
          {images.map((image, indx)=>{return(
            <Grid item xs={6} key={`${image}-${indx}`}>
              {`${retrievePath()}${image}.jpg`}
              <PreloadImage src={`${retrievePath()}${image}.jpg`} alt="Simple, macro-calculated recipes" style={{width: '100%'}}/>
            </Grid>
          )})}
          {/* <Grid item xs={6}>
